...
這個 gate 不是完整 integration test，也不打真實金流或真實訂單。它只確認 contract docs、Tachiya router surface、Storefront consumer helper、以及 Storefront consumer tests 仍同時存在並含有關鍵欄位 / URL / header。若未來 Storefront 改檔名或 API contract 改 endpoint，必須同一張 PR 更新本文件與 sanity test。
...
- Cross-repo gate fail 時，優先判斷是 contract drift、檔案移動未更新測試、或 checkout/token 問題。
