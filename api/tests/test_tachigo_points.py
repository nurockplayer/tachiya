...
@pytest.mark.anyio
async def test_get_user_points_trims_configured_internal_secret(monkeypatch):
    requests: list[httpx.Request] = []
...
@pytest.mark.anyio
async def test_get_user_points_rejects_string_balances(monkeypatch):
    settings = Settings(tachigo_api_url="http://tachigo.local")
    monkeypatch.setenv("TACHIYA_INTERNAL_SHARED_SECRET", "shared-secret")
...
@pytest.mark.anyio
async def test_get_identity_points_rejects_boolean_balances(monkeypatch):
    settings = Settings(tachigo_api_url="http://tachigo.local")
    monkeypatch.setenv("TACHIYA_INTERNAL_SHARED_SECRET", "shared-secret")
